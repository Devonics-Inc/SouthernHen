...
w = 295-- Width of 20lb green  box
l = 399 --Length of 20lb green box
h = 240 -- Height of 20lb green box
...
-- REAL VALUES:
-- max_layer_low = 3 -- At this layer, move rail to mid
-- max_layer_mid = 5 -- At this layer, move rail to high
-- max_layers = 8 -- Max layers in the pallet (minus 1 for the first layer)

-- TEST VALUES:
max_layer_low = 1 -- At this layer, move rail to mid
max_layer_mid = 3 -- At this layer, move rail to high
    ...
    while(side == LEFT) do
        ...
        if(GetDI(5, 0) == 1 or GetDI(5,0) == 0) then -- This needs to be "if(box detected and other robot is ready)"
            if (pattern == 1) then
                ...
                NewDofile("/fruser/left_pattern_1_large.lua", 1, 1)
				DofileEnd()
            ...
            elseif (pattern == 2) then
                ...
                SetSysVarValue(s_var_1, layer)        
                SetSysVarValue(s_var_2, rail_pos)
                NewDofile("/fruser/left_pattern_2_large.lua",1,2)
    			DofileEnd()
    			if(layer >= pause_layer - 1) then
    			    pattern = 1
    			end
    ...
    while(side == RIGHT) do
        ...
        if(GetDI(5, 0) == 1 or GetDI(5,0) == 0) then -- This needs to be "if(box detected and other robot is ready)"
            if (pattern == 1) then
                ...
                SetSysVarValue(s_var_1, layer)
                SetSysVarValue(s_var_2, rail_pos)
                NewDofile("/fruser/right_pattern_1_large.lua", 1, 3)
				DofileEnd()
            ...
            elseif (pattern == 2) then
                ...
                SetSysVarValue(s_var_1, layer)        
                SetSysVarValue(s_var_2, rail_pos)
                NewDofile("/fruser/right_pattern_2_large.lua",1,4)
    			DofileEnd()
    			if(layer >= pause_layer) then
    			    pattern = 1
    			end
