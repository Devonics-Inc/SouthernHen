...
while(1) do
    ...
    if(box_num < 7) then
        ...
        x_offset = -1 * ((2 * w) + (l - w) / 2)
        y_offset = -1 * (w * (box_num - 7)) + ((l - w) / 2)
    end

    PTP(LeftMidPt20lb_large,100,500,1, 0, 0, z_offset,0,0,rot_offset)-- Go to mid pt (might need an ofset in the field)
    -- ...or here. Depends on how much faith you have in max speed
    -- Place box in prep spot (10mm offset in all directions)
    PTP(LeftBIC20lb_large, 100,500,1, x_offset - 30 ,y_offset - 10, z_offset + 50, 0, 0, rot_offset)
    Lin(LeftBIC20lb_large,100,500,0,1,x_offset,y_offset,z_offset-10,0,0,rot_offset)
    SetDO(1,0,0,0) -- Release gripper
    WaitMs(300) -- Wait to ensure vacuum is released
    Lin(LeftBIC20lb_large,100,500,0,1,x_offset,y_offset, z_offset+100,0,0,rot_offset)
    PTP(LeftPrep20lb_large,100,0,1,0,0, z_offset, 0,0,0)
    box_num = box_num + 1
    if box_num > 1  and box_num < 7 then
        if(row == 2) then
            row = 0
            if(col == 1) then
                col = 0
